...
export class GameScene extends Phaser.Scene {
  ...
  private updatePlayer(time: number, delta: number) {
    ...
    // Movement
    const vx = (this.wasd.A.isDown ? -1 : 0) + (this.wasd.D.isDown ? 1 : 0);
    const vy = (this.wasd.W.isDown ? -1 : 0) + (this.wasd.S.isDown ? 1 : 0);
    ...
    if (this.isReloading) {
      ...
      // Manual reload
      if (Phaser.Input.Keyboard.JustDown(this.wasd.R) && this.playerAmmo < WEAPONS[this.playerWeaponKey].maxAmmo) {
        this.startReload();
      }
      // Auto reload
  ...
  private tryPickup() {
    ...
    if (nearest) {
      ...
      this.playerWeaponKey = nearest.key;
      this.playerAmmo = WEAPONS[nearest.key].ammo;
      this.playerReserves = WEAPONS[nearest.key].reserves;
      this.isReloading = false;
    }
  }
